    ...
    with tab1:
        ...
        if likert_cols:
            ...
            c2.metric(
                ...
                f"{overall:.2f}" if pd.notna(overall) else "—",
            )
        else:
        ...
        if yesno_cols:
            pct_yes = (
                pd.to_numeric(f[yesno_cols].stack(), errors="coerce").mean() * 100
            )
            c3.metric(
                "% Sí (Sí/No)",
                f"{pct_yes:.1f}%" if pd.notna(pct_yes) else "—",
            )
        else:
            c3.metric("% Sí (Sí/No)", "—")
        ...
        rows = []
        for (sec_code, sec_name), g in mapa_ok.groupby(
            ["section_code", "section_name"]
        ):
            cols = [
                c
                for c in g["header_num"].tolist()
        ...
        if rows:
            sec_df = pd.DataFrame(rows).sort_values(
                "Promedio", ascending=False
            )
            st.dataframe(
                sec_df.drop(columns=["sec_code"]),
    ...
    with tab2:
        ...
        sec_df2 = (
            pd.DataFrame(rows).sort_values("Promedio", ascending=False)
            if rows
            else pd.DataFrame()
        )
        ...
        for _, r in sec_df2.iterrows():
            with st.expander(
                f"{r['Sección']} — Promedio: {r['Promedio']:.2f}",
                expanded=False,
            ):
    ...
    if vista == "Dirección General" and tab_comp is not None:
        with tab_comp:
            ...
            if not carrera_col:
                st.warning(
                    "No se encontró una columna válida para identificar la carrera."
                )
                st.stop()
    ...
    with tab3:
        ...
        if not open_cols:
            st.info(
                "No detecté columnas de comentarios con la heurística actual."
            )
            return

        col_sel = st.selectbox(
            "Selecciona el campo a revisar", open_cols
        )
