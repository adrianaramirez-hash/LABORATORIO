...
def _get_service_account_dict():
    """
    Acepta 2 formas:
    1) secrets.toml con [gcp_service_account_json] como tabla (dict)
    2) secrets.toml con gcp_service_account_json = "{...}" como string JSON
    """
    ...
    # Streamlit secrets suele regresar un AttrDict-like; lo convertimos a dict normal
...
def render_encuesta_calidad(vista=None, carrera=None):
    ...
    try:
        ...
        # 5) Merge catálogo (Virtual/Escolar)
        ...
        with tab1:
            ...
            if sec_df.empty:
                st.info("No hay datos suficientes para calcular promedios por sección.")
                st.stop()
        ...
        st.error("Ocurrió un error al cargar/mostrar Encuesta de calidad.")
        st.exception(e)
        st.stop()
